...
describe("processVendingSessions", () => {
	...
	it("should handle cancel operation", () => {
		const input = {
			...
			sessions: [[["insert", 25], ["insert", 25], ["cancel"]]],
		};
	...
	it("should handle unknown actions", () => {
		const input = {
			...
			sessions: [[["unknown"], ["insert", 50], ["select", "K"]]],
		};
	...
	it("should handle cancel after session ends", () => {
		const input = {
			...
			sessions: [[["insert", 25], ["cancel"], ["insert", 50]]],
		};
		...
		expect(result.receipts).toEqual([
			{
				...
				errors: [
					"unsupported coin: 75",
					"invalid sku: INVALID",
					"insufficient credit: have 50, need 100",
				],
			},
		]);
	});
