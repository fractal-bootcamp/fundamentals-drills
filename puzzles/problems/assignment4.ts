...
function stringToArray(line: string) {
    let finalArray = []
    let currentWord = ""
...
function getWayPointName(i: number, array: string[]) {

    let wayPointName = ""
    for (let j = i + 1; j < array.length; j++) {
        if (array[j] != "F" && array[j] != "G" && array[j] != "L" && array[j] != "R" && array[j] != "B" && array[j] != "M") {
...
export function navigateRobot(commands) {
    ...
    for (let i = 0; i < directions.length; i++) {
        ...
        console.log("PREV SQUARES", previousSquares)
